...
module.exports.action = async (msg, args) => {
  if (commandeFormat.split(" ").length <= args.length) {
    ...
    try {
      await msg.guild.members.kick(member);
      const kicked = {
        title: `\`${member.user.tag}\` a été ***__expulsé(e)__*** avec succès !`,
        color: Colors.Green,
        description: `${member.user.tag} à été expulsé(e) avec succès par ${msg.author.tag}`,
      };

      msg.channel.send({ embeds: [kicked] });
    } catch (error) {
      msg.channel.send(
        "Une erreur est survenue lors de l'exclusion de ce membre. Vous pouvez vérifier mes permissions sur le serveur puis réessayer."
      );
    }
  } else {
    await msg.reply(
